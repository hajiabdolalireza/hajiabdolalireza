<p align="center">
  <img src="assets/alireza-hajiabdolali-logo.png" alt="Alireza Hajibdolalireza Logo"width="290"/>
</p>

# 👋 Hi, I'm Alireza Hajiabdolali 👨🏻‍💻

**BizOps | Product Strategy | Product Management**
  
I work at the intersection of **business, operations, and product**.

My background combines **BizOps / COO experience, product management, business analysis, system design, and hands-on web app prototyping**. I’m especially interested in building and improving products through a **system-oriented and data-driven approach**: understanding the problem clearly, defining the right metrics, prioritizing effectively, and driving execution toward measurable outcomes.

I’m particularly motivated by:
- **Entrepreneurship**
- **Product Strategy**
- **Operations & Growth**
- **Profitability-focused execution**
- **Designing better systems for better decisions**

---

## 🚀 What I Focus On

### 📌 Product Strategy & Product Management
I enjoy shaping products from problem definition to execution:
- Product vision and roadmap definition
- MVP thinking and success metrics
- KPI/OKR design and tracking
- Prioritization using frameworks like **MoSCoW**
- Agile execution and release planning
- Cross-functional coordination between product, operations, marketing, and tech

### 📊 BizOps, Operations & Profitability
A major part of my work is improving how businesses operate:
- Designing lightweight operational and financial systems
- Portfolio and profitability analysis
- Cost control and better resource allocation
- Turning messy operations into structured, trackable workflows
- Helping teams make better decisions with clearer business visibility

### 🧠 Data-Driven Decision Making
I like using data as a decision tool, not just a reporting layer:
- KPI definition and performance monitoring
- Management reporting
- Revenue/cost analysis
- Customer persona building
- User journey and touchpoint analysis
- Using business signals to guide product and operational decisions

### 🔄 Process Design & Automation
I’m interested in reducing manual work and increasing execution speed:
- Workflow design
- Process improvement
- Automation of repetitive tasks
- Using **AI tools** and **n8n** for operational leverage
- Building simple systems that improve team productivity and decision quality

### 🎯 Product Design & User Understanding
I care about product decisions being grounded in user understanding:
- UX research
- Persona definition
- User flow design
- Journey mapping
- Wireframe / prototype collaboration
- Improving UI/UX in alignment with business and product goals

### 🛠 Technical Prototyping & System Thinking
While my main direction is product and strategy, I also use technical skills to think better and prototype faster:
- Web app architecture
- System design
- Full-stack prototyping for testing ideas
- Working with **Go**, **JavaScript/TypeScript**, and **Python**
- Using tools like **Postman**, **Git/GitHub**, and **WordPress/Elementor**

---

## 💼 Selected Experience

### COO — Asonsazeh
I’ve been focused on improving profitability, operational clarity, and execution quality by:
- Implementing financial systems for better visibility and decision-making
- Analyzing portfolio performance using **BCG Matrix**
- Reallocating focus toward higher-margin products
- Removing loss-making products
- Aligning innovation, marketing, and SEO around priority products
- Defining customer personas and user journeys
- Automating repetitive workflows with **AI + n8n**

**Selected outcomes:**
- **15% average sales growth** in the first 6 months
- **4% margin growth**
- Better speed and quality in operational decision-making

### Product Manager & Business Analyst — Sepehr Boom Rad
I worked on both product and execution:
- Product roadmap development
- KPI definition
- Prioritization with **MoSCoW**
- Operational follow-ups and project tracking
- Refactoring legacy code and shipping new features
- Collaborating on rebranding and marketing strategy
- Persona, touchpoint, and journey analysis
- UI/UX improvement

**Selected outcomes:**
- **30% reduction in development and maintenance cost**
- **Product launch delivered 2 months earlier** through agile execution

### Freelance Consultant — Product / Business / Finance
I’ve also worked with startups and agile teams on:
- Business diagnostics and growth challenges
- Product consulting
- Financial system design
- Cost control
- MVP and roadmap definition
- UX research and product design support
- Technical prototyping and system architecture
- SEO prioritization aligned with profitability and product goals

---

## 🧰 Core Skills

### Product / Strategy / BizOps
- Product Strategy
- Product Management
- BizOps
- KPI / OKR
- Agile Execution
- MoSCoW Prioritization
- GTM Strategy
- Stakeholder Management
- Process & Ops Design
- Portfolio & Profitability Analysis
- BCG Matrix
- Data-Driven Decision Making

### Business & Finance
- Management Reporting
- Cost Control
- Revenue / Cost Modeling
- SME Finance Systems
- Advanced Excel

### Product Design
- UX Research
- Persona Definition
- User Journey Mapping
- User Flow Design
- Wireframing / Prototyping
- UI/UX Improvement

### Tech & Prototyping
- Web App Architecture
- System Design
- Go
- JavaScript / TypeScript
- Python
- Git / GitHub
- Postman
- WordPress / Elementor
- AI Tools & Agents
- n8n

---

## 🎓 Education
- **M.A. in Entrepreneurship (New Venture Business)** — University of Tehran *(ongoing)*
- **B.A. in Business Management** — Allameh Tabataba’i University

---

## 🌱 Current Direction
Right now, I’m intentionally moving deeper into:
- **Product Management**
- **Product Strategy**
- **BizOps for startups**
- **Operations in agile organizations**
- **Entrepreneurship and building scalable businesses**

I’m especially excited by roles where I can connect:
**user needs + business goals + operational execution**

---

## 🤝 Let’s Connect
If you care about building products with a strong mix of:
- strategic thinking,
- business understanding,
- operational discipline,
- and product sense,

I’d be happy to connect and exchange ideas.


---

---

## 🎯 Quick Facts   
- 💼 Role: **Full-Stack Developer & Software Architect**  
- ♟️ Hobbies: **Chess · Books**  
- 📚 Currently Learning: **Advanced Web3 & Blockchain**  

---





<!--START_SECTION:waka-->
**I'm an Early 🐤** 

```text
🌞 Morning                88 commits          ████████████░░░░░░░░░░░░░   48.89 % 
🌆 Daytime                46 commits          ██████░░░░░░░░░░░░░░░░░░░   25.56 % 
🌃 Evening                26 commits          ████░░░░░░░░░░░░░░░░░░░░░   14.44 % 
🌙 Night                  20 commits          ███░░░░░░░░░░░░░░░░░░░░░░   11.11 % 
```


📊 **This Week I Spent My Time On** 

```text
🕑︎ Time Zone: Asia/Tehran

💬 Programming Languages: 
No Activity Tracked This Week

🐱‍💻 Projects: 
No Activity Tracked This Week
```


<!--END_SECTION:waka-->

---

## ⚡ Recent Activity

<!--RECENT_ACTIVITY:start-->
1. ⬆️ Pushed undefined commit(s) to [hajiabdolalireza/hajiabdolalireza](https://github.com/hajiabdolalireza/hajiabdolalireza)<br>
2. ⬆️ Pushed undefined commit(s) to [hajiabdolalireza/hajiabdolalireza](https://github.com/hajiabdolalireza/hajiabdolalireza)<br>
3. ⬆️ Pushed undefined commit(s) to [hajiabdolalireza/hajiabdolalireza](https://github.com/hajiabdolalireza/hajiabdolalireza)<br>
4. ⬆️ Pushed undefined commit(s) to [hajiabdolalireza/hajiabdolalireza](https://github.com/hajiabdolalireza/hajiabdolalireza)<br>
5. ⬆️ Pushed undefined commit(s) to [hajiabdolalireza/hajiabdolalireza](https://github.com/hajiabdolalireza/hajiabdolalireza)<br>
<!--RECENT_ACTIVITY:end-->

---

### 🐱 My GitHub Data
![Metrics](/github-metrics.svg)


---

## 🎶 Spotify Now Playing
[![spotify-github-profile](https://spotify-github-profile-one-jet.vercel.app/api/view?uid=31smmp3iinzo7fav4vohfh5qudwu&cover_image=true&theme=novatorem&show_offline=true&background_color=121212&interchange=true&bar_color=53b14f&bar_color_cover=true)](https://spotify-github-profile-one-jet.vercel.app/api/view?uid=31smmp3iinzo7fav4vohfh5qudwu&redirect=true)

---

### 🚧 my todoist stats:

<!-- TODO-IST:START -->
🏆  1,762 Karma Points           
🌸  Completed 0 tasks today           
🗓  Completed 0 tasks this week           
✅  Completed 70 tasks so far           
⏳  Longest streak is 4 days
<!-- TODO-IST:END -->

---

## 📊 GitHub Stats
![Alireza's GitHub stats](https://github-readme-stats.vercel.app/api?username=hajiabdolalireza&show_icons=true&theme=radical)
